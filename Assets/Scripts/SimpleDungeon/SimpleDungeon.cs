...
{
    [Header("Dungeon")]
    [Min(2)] public int roomCount = 8;                      //방의 개수 최소 값은 2
    [Min(3)] public int minRoomSize = 4;                    //최소 사이즈 4
    [Min(3)] public int maxRoomSize = 7;                    //최대 사이즈 
    [Min(5)] public int roomSpacing = 8;                   
    [Min(10)] public int maxPlacementAttempts = 200;        //생성 방이 중복될때 최대 시도 횟수
    ...
    private void AssignSpecialRooms()           //생성된 일반 방 중에서 보스방과 보물방을 지정 합니다. 
    {
        ...
        foreach(Room room in rooms.Values)      //생성된 모든 방을 검사 
        {
            ...
            int distance = Mathf.Abs(room.center.x) + Mathf.Abs(room.center.y); //원점에서 방 줌심 가지 거리 계산
